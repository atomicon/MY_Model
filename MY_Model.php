...
class MY_Model extends CI_Model
{
    ...
    // variables set if they do not exist (php 8.3)
    public $variables = [];
        ...
        if ($this->_database_connected()) {
            ...
            if (!empty($this->slug_key) && $this->has_field($this->slug_key)) {
                if (empty($data[$this->slug_key])) {
                    $this->stored[$this->slug_key] = $data[
                        $this->slug_key
                    ] = $this->_slugify();
                } else {
            ...
            if ($this->exists()) {
                if (array_key_exists("updated", $data)) {
                    $this->stored["updated"] = $data["updated"] = date(
                        "Y-m-d H:i:s"
                    );
                }
                $this->where(
                    $this->primary_key,
                    $this->stored[$this->primary_key]
                );
                $result = $this->db->update($this->table, $data);
            } else {
                if (array_key_exists("created", $data)) {
                    $this->stored["created"] = $data["created"] = date(
                        "Y-m-d H:i:s"
                    );
                }
                if (array_key_exists("updated", $data)) {
    ...
    public function __get($name)
    {
        ...
        } elseif (method_exists($this, "__get_" . $name)) {
    ...
    private function _get_table_data()
    {
        if (!is_array(MY_Model::$table_data)) {
            $cache_file = APPPATH . "cache/table_data.php";
            if (
                MY_Model::$table_data_cache_ttl > 0 &&
                file_exists($cache_file)
            ) {
                if (
                    time() - filemtime($cache_file) <
                    MY_Model::$table_data_cache_ttl
                ) {
                    include $cache_file;
                    if (!empty($table_data)) {
            ...
            if (isset(get_instance()->db)) {
                ...
                foreach ($tables as $table) {
                    MY_Model::$table_data[$table] = $this->db->field_data(
                        $table
                    );
            ...
            if (MY_Model::$table_data_cache_ttl > 0) {
                $data =
                    ...
                    PHP_EOL .
                    '$table_data = ' .
                    var_export(MY_Model::$table_data, true) .
                    ";";
    ...
    private function _get_table()
    {
        if (!is_string($this->table)) {
            ...
            $class = preg_replace(
                '#((_m|_model)$|$(m_))?#',
                "",
                strtolower($this->_get_called_class())
            );
